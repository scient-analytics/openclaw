# Interactive Sync — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rewrite the `/sync` Claude Code command as an interactive interview that captures rich session context, detects skill changes, offers skill export, and sends an enriched payload to Michael.

**Architecture:** Single file rewrite of `skills/sync-to-michael/sync.md`. The skill is a markdown instruction file for Claude Code — no compiled code, no tests. The interview logic is expressed as step-by-step instructions that Claude follows conversationally. Skill creation follows the `skill-creator` conventions from the repo.

**Tech Stack:** Markdown (Claude Code command format), bash (curl for HTTP, python for JSON), env vars for config.

---

### Task 1: Rewrite sync.md with the interactive interview flow

**Files:**

- Modify: `skills/sync-to-michael/sync.md`

**Reference docs:**

- Design: `docs/plans/2026-02-23-interactive-sync-design.md`
- Skill creator conventions: `skills/skill-creator/SKILL.md` (for how to draft skills)
- Current sync: `skills/sync-to-michael/sync.md` (what we're replacing)

**Step 1: Replace the full content of `skills/sync-to-michael/sync.md`**
...
Expected: frontmatter starting with `description: Sync session to Michael`

**Step 3: Commit**

```bash
git add skills/sync-to-michael/sync.md
git commit -m "feat(michael): rewrite /sync as interactive debrief with skill export"
```

---

### Task 2: Test the interactive sync with a real session

This task is manual — run `/sync` in the current Claude Code session and verify the interview flow works end to end.

**Step 1: Ensure env vars are set**

```bash
echo "URL: $MICHAEL_URL"
echo "TOKEN: $MICHAEL_TOKEN"
echo "NAME: $MICHAEL_NAME"
```

All three should be non-empty.

**Step 2: Run `/sync` in the current session**

Type `/sync` in Claude Code. Verify:

- [ ] Claude proposes a goal and waits for confirmation
- [ ] Claude lists tools/skills detected and asks for additions
- [ ] Claude proposes outcomes and waits for validation
- [ ] Claude checks for skill changes (modified, new, patterns)
- [ ] Claude shows relevant good-practice nudges (if any)
- [ ] Claude builds and shows the payload before sending
- [ ] Payload is sent to Michael via gateway

**Step 3: Verify Michael received it**
...
Check that Michael's session has the enriched sync message.
...
```bash
git add skills/sync-to-michael/sync.md
git commit -m "fix(michael): adjust interactive sync based on testing"
```
